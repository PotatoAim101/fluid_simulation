...
int main()
{
	...
	while(window.isOpen())
	{
		...
		if(left && Lctrl) fluid.setWallZone(mousePos,8,0);
		else if(left) fluid.setWallZone(mousePos,8,1);

		if(right && S) { fluid.addSpeedZone(Vector2f(0,55), Vector2f(width,30), Vector2f(0,5)); }
		if(right && P)fluid.addPressureZone(mousePos, 30, 100000); 
		if(right && N)fluid.addPressureZone(mousePos, 30, -100000); 
		...
		else if(S) window.draw(fluid.getSpeedSprite());
		else if(P) window.draw(fluid.getPressureSprite());
		else if(N) window.draw(fluid.getPressureSprite());
		else if(C) window.draw(fluid.get3DSprite());
		else window.draw(fluid.getDensitySprite());
		
		window.draw(fluid.getWallSprite());

		if(D) fluid.drawSpeedField(window, 10, Color::White);
